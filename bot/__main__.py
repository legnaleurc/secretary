...
class UpdateHandler(api.BotHookHandler):
    ...
    async def on_text(self, message):
        ...
        for handler in lich.text_handlers:
            ...
            if result:
                result = await result
                await lich.client.send_message(
                    chat.id_, result, reply_to_message_id=id_
                )
                break
        else:
...
class TwitchPuller(object):
    ...
    def __init__(self, kel_thuzad, channel_name):
        ...
        self._request = httpclient.HTTPRequest(
            url,
            headers={
    ...
    async def __call__(self):
        ...
        if self._is_online:
            ...
            if data["stream"] is not None:
                ...
                await self._kel_thuzad.client.send_message(
                    self._chat_id,
...
async def shell_out(*args, **kwargs):
    ...
    if stdin is not None:
        p = process.Subprocess(
            args, stdin=process.Subprocess.STREAM, stdout=process.Subprocess.STREAM
        )
...
def main(args=None):
    ...
    options.define(
        ...
        metavar="telezombie.yaml",
        callback=parse_config,
    )
    options.define(
        "api_token", default=None, type=str, help="API token", metavar="<token>"
    )
    options.define(
        "database", default=None, type=str, help="database URI", metavar="<uri>"
    )
